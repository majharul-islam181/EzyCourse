  ...
  Exception _parseDioError(final DioException error) {
    switch (error.type) {
      ...
      case DioExceptionType.badResponse:
        ...
        if (responseData is Map<String, dynamic>) {
          ...
          if (responseData['errors'] is Map<String, dynamic>) {
            validationErrors = (responseData['errors'] as Map<String, dynamic>).map(
              (final key, final value) {
                if (value is List) {
                  return MapEntry(key, value.map((final e) => e.toString()).toList());
                }
      ...
      case DioExceptionType.badCertificate:
        return const ServerException(message: 'Bad certificate. Secure connection failed.');
      case DioExceptionType.unknown:
      default:
        return const NetworkException(message: 'An unexpected network error occurred.');
    }
  }
}
